...
latest="$(find "$BACKUP_DIR" -maxdepth 1 -type f -name 'jalwa-postgres-*.dump' -printf '%T@ %p\n' | sort -nr | head -1 | cut -d' ' -f2-)"
if [[ -z "$latest" ]]; then
  ...
  [[ -n "$key" ]] || { echo "No PostgreSQL backup is available for a restore drill." >&2; exit 1; }
...
[[ -s "$latest" && -s "$latest.sha256" ]] || { echo "Backup or checksum is missing." >&2; exit 1; }
expected="$(cut -d' ' -f1 "$latest.sha256")"
actual="$(sha256sum "$latest" | cut -d' ' -f1)"
[[ "$expected" == "$actual" ]] || { echo "Backup checksum mismatch." >&2; exit 1; }
docker exec -i "$DB_CONTAINER" pg_restore --list < "$latest" >/dev/null
...
docker exec -i "$DB_CONTAINER" pg_restore -U postgres -d "$drill_db" --exit-on-error --no-owner --no-acl < "$latest"
...
printf '%s\n' "$(date -u +%FT%TZ) $(basename "$latest") $actual" > "$BACKUP_DIR/LAST_RESTORE_DRILL"
echo "Restore drill succeeded using $(basename "$latest")"
