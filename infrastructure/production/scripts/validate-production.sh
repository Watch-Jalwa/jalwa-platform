...
require_match 'required=(DEPLOYMENT_ENVIRONMENT DATABASE_URL BETTER_AUTH_SECRET R2_ACCOUNT_ID' .github/workflows/deploy-staging.yml 'Staging does not require direct PostgreSQL and Better Auth runtime configuration.'
require_match 'upsert_env BETTER_AUTH_URL "https://$DOMAIN"' .github/workflows/deploy-staging.yml 'Staging Better Auth public URL normalization is missing.'
require_match 'upsert_env NODE_ENV production' .github/workflows/deploy-staging.yml 'Staging does not enforce production runtime behavior.'
...
require_match 'expected_volume=jalwa-platform_jalwa_staging_pgdata' .github/workflows/deploy-staging.yml 'Codistan staging does not pin the existing PostgreSQL volume.'
require_match 'expected_network=jalwa-platform_default' .github/workflows/deploy-staging.yml 'Codistan staging does not pin the existing Docker network.'
require_match "export JALWA_COMPOSE_SERVICES='web worker'" .github/workflows/deploy-staging.yml 'Codistan staging adoption must replace application services only.'
require_match 'export JALWA_DEPLOY_NO_DEPS=true' .github/workflows/deploy-staging.yml 'Codistan staging adoption must preserve the existing PostgreSQL dependency.'
require_match 'name: jalwa-platform_jalwa_staging_pgdata' infrastructure/production/docker-compose.codistan-staging.yml 'Codistan staging overlay does not preserve the existing pgdata volume.'
require_match 'name: jalwa-platform_default' infrastructure/production/docker-compose.codistan-staging.yml 'Codistan staging overlay does not preserve the existing Docker network.'
require_match '127.0.0.1:${JALWA_WEB_PORT:-3001}:3000' infrastructure/production/docker-compose.codistan-staging.yml 'Codistan staging nginx loopback boundary is missing.'
