...
for script in infrastructure/production/scripts/*.sh; do
  bash -n "$script"
done

if grep -R -n -E 'SUPABASE_ACCESS_TOKEN|api\.supabase\.com/v1/projects|supabase projects create' \
  .github/workflows infrastructure/production --exclude='validate-production.sh'; then
...
for key in SELF_HOSTED_POSTGRES_PASSWORD SELF_HOSTED_SUPABASE_JWT_SECRET SELF_HOSTED_SUPABASE_ANON_KEY SELF_HOSTED_SUPABASE_SERVICE_ROLE_KEY; do
...
trap 'rm -f infrastructure/production/.env.production /tmp/jalwa-self-hosted-secrets.env' EXIT
(
...
for migration in "${migrations[@]}"; do
  [[ "$migration" =~ ^[0-9]{12,}_[a-z0-9_]+\.sql$ ]] || {
