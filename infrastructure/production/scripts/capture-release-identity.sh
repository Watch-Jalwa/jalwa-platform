...
pipeline_id="${2:?deployment pipeline ID is required}"
...
[[ "$pipeline_id" =~ ^[0-9]+$ ]] || { echo "Pipeline ID must be numeric." >&2; exit 1; }
...
service_json() {
  ...
  [[ "$build_run_id" == "$pipeline_id" ]] || { echo "Build pipeline label mismatch for $service." >&2; return 1; }
...
jq -n \
  ...
  --arg deployment_pipeline_id "$pipeline_id" \
  --arg rollback_release "$rollback_release" \
  --argjson web "$web" \
  --argjson worker "$worker" \
  '{schema_version:$schema_version,captured_at:$captured_at,expected_source_sha:$expected_source_sha,deployment_pipeline_id:$deployment_pipeline_id,rollback_release:($rollback_release | if length == 0 then null else . end),services:{web:$web,worker:$worker}}' > "$temporary"
chmod 600 "$temporary"
