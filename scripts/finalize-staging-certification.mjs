...
async function readArea(area) {
  try {
    ...
    if (payload?.area !== area || !allowedStatuses.has(payload?.status) || typeof payload?.summary !== "string" || !Number.isInteger(payload?.test_count) || payload.test_count < 1) {
...
const sumTests = (status) => areas.filter((item) => item.status === status).reduce((sum, item) => sum + item.test_count, 0);
const report = {
  schema_version: 1,
  ...
  test_totals: {
    pass: sumTests("PASS"),
    fail: sumTests("FAIL"),
    blocked: sumTests("BLOCKED"),
    not_applicable: sumTests("N/A"),
    total: areas.reduce((sum, item) => sum + item.test_count, 0),
  },
  areas,
};
