...
import { downloadObject, run, uploadDirectory } from "./media.mjs";
...
export function buildPackagerArgs({ inputDir, outputDir, keyId, key, iv, protectionSystems = "Widevine,FairPlay" }) {
  ...
  const labels = ["SD","HD","AUDIO"].map((label) => `label=${label}:key_id=${keyId}:key=${key}${iv?`:iv=${iv}`:""}`).join(",");
  return [...descriptors,"--enable_raw_key_encryption","--keys",labels,"--protection_scheme","cbcs","--protection_systems",protectionSystems,"--hls_key_uri",`skd://${keyId}`,"--segment_duration","6","--hls_master_playlist_output",join(outputDir,"master.m3u8"),"--mpd_output",join(outputDir,"manifest.mpd")];
}
...
export async function processDrmPackagingJob({ supabase, job }) {
  ...
  const sourceKey = asset.media_assets?.storage_key;
  ...
  try {
    ...
    await run(ffmpeg,buildAudioArgs(source,join(encoded,"audio.mp4")));
    const material = await packagingKey(asset);
    await run(process.env.SHAKA_PACKAGER_PATH ?? "packager",buildPackagerArgs({inputDir:encoded,outputDir:output,...material}));
    ...
    await supabase.from("drm_packaging_jobs").update({status:"completed",completed_at:new Date().toISOString(),output:{manifestHls,manifestDash,uploaded}}).eq("id",job.id);
