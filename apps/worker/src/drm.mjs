...
export function buildVideoArgs(input, output, height) {
  return ["-y", ...inputArgs(input), "-map", "0:v:0", "-an", "-vf", `scale=-2:${height}`, "-c:v", "libx264", "-preset", "veryfast", "-crf", height >= 720 ? "22" : "24", "-g", "144", "-keyint_min", "144", "-sc_threshold", "0", "-movflags", "+faststart", output];
}

export function buildAudioArgs(input, output) {
  return ["-y", ...inputArgs(input), "-map", "0:a:0", "-vn", "-c:a", "aac", "-b:a", "128k", "-movflags", "+faststart", output];
}
...
async function packagingKey(asset) {
  ...
  const response = await fetch(endpoint, {
    ...
    body: JSON.stringify({ drmAssetId: asset.id, contentId: asset.content_id, mediaAssetId: asset.media_asset_id, provider: asset.provider, providerAssetRef: asset.provider_asset_ref }),
    signal: AbortSignal.timeout(Number(process.env.DRM_PROVIDER_TIMEOUT_MS ?? 15000)),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.error ?? `DRM key service failed (${response.status}).`);
  return {
    keyId: assertHex(data.keyId, "keyId"),
    key: assertHex(data.key, "key"),
    iv: data.iv ? assertHex(data.iv, "iv") : null,
    providerAssetRef: typeof data.providerAssetRef === "string" ? data.providerAssetRef : asset.provider_asset_ref,
    protectionSystems: typeof data.protectionSystems === "string" ? data.protectionSystems : "Widevine,FairPlay",
  };
}
...
export async function processDrmPackagingJob({ supabase, job }) {
  ...
  const root = join(process.env.MEDIA_TEMP_DIR ?? "/tmp/jalwa-media", `drm-${job.id}`);
  ...
  try {
    ...
    const prefix = `drm/${asset.content_id}/${asset.id}/`;
