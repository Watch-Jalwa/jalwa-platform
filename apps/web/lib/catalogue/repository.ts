...
function hasDatabaseConfiguration() { return Boolean(process.env.NEXT_PUBLIC_SUPABASE_URL && process.env.NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY); }
...
function mapSearchRow(row: Record<string, unknown>): CatalogueItem { return { id: asString(row.id) ?? undefined, slug: asString(row.slug) ?? "unavailable", title: asString(row.title) ?? "Untitled", titleUrdu: asString(row.title_ur), description: asString(row.description), category: asString(row.category_name) ?? "Jalwa", categorySlug: asString(row.category_slug) ?? "all", durationSeconds: asNumber(row.duration_seconds), accessLevel: (asString(row.access_level) ?? "public") as CatalogueItem["accessLevel"], contentType: (asString(row.content_type) ?? "video") as CatalogueItem["contentType"], hostingMode: (asString(row.hosting_mode) ?? "external_link") as CatalogueItem["hostingMode"], thumbnailUrl: asString(row.thumbnail_url) }; }

export async function getCategories(): Promise<CatalogueCategory[]> {
  if (!hasDatabaseConfiguration()) return demoCategories;
  try { const supabase = await createClient(); const { data, error } = await supabase.from("categories").select("slug,name_en,name_ur,name_roman_ur,icon").eq("is_active", true).order("sort_order"); if (error) throw error; return (data ?? []).map((row) => ({ slug: row.slug, label: row.name_en, urdu: row.name_ur, romanUrdu: row.name_roman_ur, icon: row.icon })); } catch { return demoCategories; }
}

export async function searchCatalogue(input: { query?: string; category?: string; limit?: number } = {}): Promise<CatalogueItem[]> {
  const query = input.query?.trim() ?? ""; const category = input.category?.trim() ?? "";
  if (!hasDatabaseConfiguration()) return featuredContent.filter((item) => { const categoryMatches = !category || item.categorySlug === category; const haystack = `${item.title} ${item.titleUrdu ?? ""} ${item.description ?? ""}`.toLowerCase(); return categoryMatches && (!query || haystack.includes(query.toLowerCase())); });
  try { const supabase = await createClient(); const { data, error } = await supabase.rpc("search_catalogue", { p_query: query || null, p_category: category || null, p_limit: input.limit ?? 40 }); if (error) throw error; return ((data ?? []) as Record<string, unknown>[]).map(mapSearchRow); } catch { return featuredContent; }
}
...
export async function getContentBySlug(slug: string): Promise<CatalogueItem | null> {
  ...
  try {
    ...
    const { data, error } = await supabase.from("content_items").select("id,slug,title_en,title_ur,description_en,content_type,hosting_mode,access_level,duration_seconds,thumbnail_url,primary_category_id").eq("slug", slug).single();
