...
export function playbackUrls(uidOrToken: string) {
  const { customerCode } = config();
  const base = `https://customer-${customerCode}.cloudflarestream.com/${uidOrToken}/manifest`;
  return { hls: `${base}/video.m3u8`, dash: `${base}/video.mpd` };
}

export async function createCloudflareLiveInput(input: { name: string; recording: boolean; allowedOrigin: string; signed: boolean; lowLatency?: boolean }) {
  const result = await request<CloudflareLiveInput>("/live_inputs", { method: "POST", body: JSON.stringify({ enabled: true, meta: { name: input.name }, preferLowLatency: Boolean(input.lowLatency), deleteRecordingAfterDays: input.recording ? 30 : undefined, recording: { mode: input.recording ? "automatic" : "off", timeoutSeconds: 0, hideLiveViewerCount: false, requireSignedURLs: input.signed, allowedOrigins: [input.allowedOrigin] } }) });
...
export async function createCloudflarePlaybackToken(uid: string, ttlSeconds = 600) {
  const result = await request<{ token?: string }>(`/${encodeURIComponent(uid)}/token`, { method: "POST", body: JSON.stringify({ exp: Math.floor(Date.now()/1000)+Math.max(60,Math.min(ttlSeconds,3600)) }) });
  if (!result.token) throw new Error("Cloudflare Stream returned no playback token.");
  return { token: result.token, playback: playbackUrls(result.token) };
}
