...
type CsvColumn = { label: string; value: string };

const columns: Record<string, CsvColumn[]> = {
...
export async function generatePremiumCsv(type: string, input: ReportInput) {
  ...
  if (type === "payments") payload = await getPaymentLedger(input, true);
    ...
    payload = await getBenefitCostReport(input);
  ...
  if (type === "summary") {
    selectedColumns = Object.keys(payload.kpis).map((key) => ({ label: key, value: key }));
  } else {
    const configuredColumns = columns[type];
    if (!configuredColumns) throw new Error("Unsupported Premium report export type.");
    selectedColumns = configuredColumns;
  }
  ...
  return {
    ...
    payload,
    filename: `jalwa-premium-${type}-${payload.effectiveRange.startDate}-to-${payload.effectiveRange.endDate}.csv`,
  };
}
...
export async function auditPremiumExport(actorId: string, type: string, result: { hash: string; rowCount: number; payload: any }) {
  const { error } = await createAdminClient().from("audit_logs").insert({
    actor_id: actorId,
    action: "premium_report_exported",
    entity_type: "premium_report",
    entity_id: type,
    metadata: {
      report_type: type,
      ...
      effective_range: result.payload.effectiveRange,
...
export function reportDefinitions() {
  return { schemaVersion: REPORT_SCHEMA_VERSION, timezone: REPORT_TIMEZONE, metricDefinitions: METRIC_DEFINITIONS };
}
