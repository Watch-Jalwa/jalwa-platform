...
export default async function DevicesPage() {
  ...
  if (!preview) {
    ...
    const { data } = await supabase.from("user_devices").select("id,display_name,platform,last_seen_at,revoked_at").eq("user_id", user.id).order("last_seen_at", { ascending: false });
    devices = data ?? [];
  }
