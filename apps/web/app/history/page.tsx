...
export default async function HistoryPage() {
  ...
  if (!preview) {
    ...
    const { data } = profile ? await database.from("watch_progress").select("content_id,position_seconds,duration_seconds,completed,last_watched_at,content_items(slug,title,title_ur,thumbnail_url)").eq("user_id", user.id).eq("viewer_profile_id", profile.id).order("last_watched_at", { ascending: false }).limit(100) : { data: [] };
