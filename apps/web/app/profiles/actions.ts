...
export async function createViewerProfile(formData: FormData) {
  ...
  const { error } = await context.database.from("viewer_profiles").insert({ user_id: context.user.id, name, profile_type: profileType, avatar_key: avatarKey, preferred_language: preferredLanguage, kids_mode: profileType === "child" });
...
export async function selectViewerProfile(formData: FormData) {
  ...
  const { data } = await context.database.from("viewer_profiles").select("id,preferred_language").eq("id", profileId).eq("user_id", context.user.id).maybeSingle();
...
export async function deleteViewerProfile(formData: FormData) {
  ...
  if (!context) redirect("/profiles?preview=1");
  const profileId = String(formData.get("profileId") ?? "");
  const { error } = await context.database.from("viewer_profiles").delete().eq("id", profileId).eq("user_id", context.user.id).eq("is_default", false);
