...
export default async function StudioLivePage() {
  ...
  const { data } = await database.from("live_channels").select("id,slug,title_en,status,provider,access_level,is_published,provider_input_id,live_events(id,title_en,scheduled_start,status)").order("updated_at", { ascending: false });
