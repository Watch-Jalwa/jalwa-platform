...
export default async function RightsOperationsPage() {
  ...
  const { data, error } = await database.from("rights_operations").select("content_id,status,creator,expires_at,is_expired,expires_within_30_days").order("expires_at", { ascending: true, nullsFirst: false }).limit(250);
