...
export default async function StudioDrmPage() {
  ...
  const [assets, content, policies, events] = await Promise.all([
    database.from("drm_assets").select("id,content_id,media_asset_id,policy_id,provider,provider_asset_ref,key_id,manifest_hls_path,manifest_dash_path,status,packaging_metadata,created_at,updated_at,content_items(slug,title_en,access_level),drm_policies(name,key_systems,minimum_security_level)").order("updated_at", { ascending: false }).limit(100),
    database.from("content_items").select("id,slug,title_en,access_level,status,media_assets(id,storage_key,status,kind),rights_records(id,status)").in("hosting_mode", ["self_host_open","self_host_owned"]).order("updated_at", { ascending: false }).limit(200),
    database.from("drm_policies").select("id,name,key_systems,licence_duration_seconds,max_concurrent_devices,minimum_security_level").order("name"),
    ["support","admin"].includes(profile.role) ? database.from("drm_license_events").select("id,drm_asset_id,key_system,status,reason,request_id,response_ms,created_at").order("created_at", { ascending: false }).limit(100) : Promise.resolve({ data: [] as unknown[], error: null }),
  ]);
