...
export default async function FinancePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  return (
    <div>
      ...
      <section className="panel"><h2>Recent checkouts</h2><div className="finance-table">{ordersResult.data?.map((order) => <div key={order.id}><span>{order.provider}</span><strong>{formatPkr(order.amount_minor)}</strong><span>{order.status}</span><time>{new Date(order.created_at).toLocaleString("en-PK", { timeZone: "Asia/Karachi" })}</time></div>)}</div></section>
