...
export default async function LiveChannelPage({ params }: { params: Params }) {
  ...
  return <div className="page-shell live-watch-page">
    <section className="live-watch-grid"><div className="player-shell">{channel.id === "demo-live" ? <div className="player-placeholder live-placeholder"><span className="live-dot" /><p>This preview channel is scheduled, not broadcasting. Production live playback activates after an input is connected.</p></div> : <LivePlayer channelId={channel.id} poster={channel.poster_url} title={channel.title_en} nextPath={`/live/${channel.slug}`} />}</div><article><span className="eyebrow">{channel.status} · {channel.access_level}</span><h1>{channel.title_en}</h1>{channel.title_ur ? <p className="urdu watch-urdu">{channel.title_ur}</p> : null}<p>{channel.description_en}</p></article></section>
