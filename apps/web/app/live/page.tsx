...
function LiveCard({ item }: { item: CatalogueItem }) {
  ...
  const linkOnly = item.playback?.deliveryAdapter === "official_live_link";
  const label = linkOnly
    ? status === "unavailable" ? "Source issue" : "Official site"
    : status === "healthy" ? "Live" : status === "off_air" ? "Off air" : status === "unavailable" ? "Unavailable" : "Checking";
...
export default async function LivePage() {
  if (!liveSourcesEnabled()) {
    return <div className="page-shell"><section className="live-hero"><span className="eyebrow">Public information</span><h1>Live sources</h1><p>The approved official live catalogue is installed but disabled in this environment.</p></section></div>;
  }
  ...
  return <div className="page-shell live-page">
    ...
    <section aria-labelledby="live-now-heading">
      <div className="section-heading"><div><span className="eyebrow">Available</span><h2 id="live-now-heading">Live and official coverage</h2></div></div>
      {liveNow.length ? <div className="live-grid">{liveNow.map((item) => <LiveCard item={item} key={item.slug} />)}</div> : <div className="empty-state">No approved source is available at the moment.</div>}
    </section>
