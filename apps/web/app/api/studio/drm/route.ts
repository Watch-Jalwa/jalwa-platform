...
async function authorized() {
  ...
  return { database, user, role: profile.role } as const;
}
...
export async function GET() {
  ...
  const [assets, content, policies, events] = await Promise.all([
    auth.database.from("drm_assets").select("id,content_id,media_asset_id,policy_id,provider,provider_asset_ref,key_id,manifest_hls_path,manifest_dash_path,status,packaging_metadata,created_at,updated_at,content_items(slug,title_en,access_level),media_assets(storage_key,status),drm_policies(name,key_systems,minimum_security_level)").order("updated_at", { ascending: false }).limit(100),
    auth.database.from("content_items").select("id,slug,title_en,access_level,status,media_assets(id,storage_key,status,kind),rights_records(id,status)").in("hosting_mode", ["self_host_open","self_host_owned"]).order("updated_at", { ascending: false }).limit(200),
    auth.database.from("drm_policies").select("id,name,key_systems,licence_duration_seconds,max_concurrent_devices,minimum_security_level").order("name"),
    ["support","admin"].includes(auth.role) ? auth.database.from("drm_license_events").select("id,drm_asset_id,key_system,status,reason,request_id,response_ms,created_at,profiles(display_name)").order("created_at", { ascending: false }).limit(100) : Promise.resolve({ data: [] as unknown[], error: null }),
  ]);
...
export async function POST(request: Request) {
  ...
  if (body.action === "retry" && body.drmAssetId) {
    const { data: asset } = await auth.database.from("drm_assets").select("id,status").eq("id",body.drmAssetId).maybeSingle();
    if (!asset) return NextResponse.json({ error: "DRM asset unavailable." }, { status: 404 });
    const { error } = await auth.database.from("drm_packaging_jobs").insert({ drm_asset_id: asset.id });
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
    await auth.database.from("drm_assets").update({ status: "pending" }).eq("id",asset.id);
  ...
  const [{ data: content }, { data: media }, { data: rights }, { data: existing }] = await Promise.all([
    auth.database.from("content_items").select("id,access_level,hosting_mode").eq("id",body.contentId).maybeSingle(),
    auth.database.from("media_assets").select("id,content_id,status,kind").eq("id",body.mediaAssetId).maybeSingle(),
    auth.database.from("rights_records").select("id").eq("content_id",body.contentId).eq("status","approved").limit(1),
    auth.database.from("drm_assets").select("id").eq("content_id",body.contentId).maybeSingle(),
  ]);
  ...
  const { data: asset, error: assetError } = await auth.database.from("drm_assets").insert({ content_id: body.contentId, media_asset_id: body.mediaAssetId, policy_id: body.policyId, provider, key_systems: { widevine: true, fairplay: true } }).select("id").single();
  if (assetError || !asset) return NextResponse.json({ error: assetError?.message ?? "DRM asset could not be created." }, { status: 400 });
  const { error: jobError } = await auth.database.from("drm_packaging_jobs").insert({ drm_asset_id: asset.id });
  if (jobError) { await auth.database.from("drm_assets").delete().eq("id",asset.id); return NextResponse.json({ error: jobError.message }, { status: 400 }); }
...
export async function PATCH(request: Request) {
  ...
  const { error } = await auth.database.from("drm_assets").update({ status }).eq("id",body.drmAssetId);
  if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  if (status === "revoked") await auth.database.from("playback_sources").update({ status: "inactive", is_primary: false }).eq("drm_asset_id",body.drmAssetId);
