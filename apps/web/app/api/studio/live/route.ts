...
export async function GET(request: Request) {
  ...
  if (!id) {
    const { data, error } = await auth.database.from("live_channels").select("*,live_events(id,title_en,scheduled_start,scheduled_end,status)").order("updated_at", { ascending: false });
    return error ? NextResponse.json({ error: error.message }, { status: 500 }) : NextResponse.json({ channels: data ?? [] });
  }
  const { data: channel } = await auth.database.from("live_channels").select("*").eq("id", id).maybeSingle();
  ...
  if (channel.provider === "cloudflare_stream" && channel.provider_input_id) {
    try {
      ...
      await auth.database.from("live_channels").update({ status, playback_hls_url: provider.playback.hls, playback_dash_url: provider.playback.dash }).eq("id", channel.id);
      await auth.database.from("live_health_checks").insert({ channel_id: channel.id, provider_status: provider.status, ingest_connected: status === "live", playback_healthy: status === "live", details: { provider: "cloudflare_stream" } });
...
export async function POST(request: Request) {
  ...
  if (body.action === "disable" && body.id) {
    const { data: channel } = await auth.database.from("live_channels").select("provider,provider_input_id").eq("id", body.id).maybeSingle();
    ...
    await auth.database.from("live_channels").update({ status: "ended", is_published: false }).eq("id", body.id);
  ...
  try {
    ...
    const { data: updated, error } = await auth.database.from("live_channels").update({ provider_input_id: created.uid, playback_hls_url: created.playback.hls, playback_dash_url: created.playback.dash, status: "offline" }).eq("id", channel.id).select("*").single();
    ...
    await auth.database.from("live_channels").delete().eq("id", channel.id);
...
export async function PATCH(request: Request) {
  ...
  const { data, error } = await auth.database.from("live_channels").update(update).eq("id", body.id).select("*").single();
