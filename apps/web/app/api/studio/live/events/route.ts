...
export async function POST(request: Request) {
  ...
  const { data, error } = await auth.database.from("live_events").insert({ channel_id: body.channelId, title_en: body.title.trim(), title_ur: body.titleUrdu?.trim() || null, description_en: body.description?.trim() || null, scheduled_start: start.toISOString(), scheduled_end: end?.toISOString() ?? null, status: body.publish ? "scheduled" : "draft" }).select("*").single();
...
export async function PATCH(request: Request) {
  ...
  const { data, error } = await auth.database.from("live_events").update(update).eq("id", body.id).select("*").single();
  return error ? NextResponse.json({ error: error.message }, { status: 400 }) : NextResponse.json({ event: data });
}
