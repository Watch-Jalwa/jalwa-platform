...
import { getPaymentServiceStatus } from "@/lib/payments/payment-service";
...
export async function GET() {
  ...
  try {
    return NextResponse.json(await getPaymentServiceStatus(userId), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
