...
export async function GET(request: Request) {
  ...
  const { data } = await supabase.from("watch_progress").select("position_seconds,completed").eq("user_id", user.id).eq("viewer_profile_id", profile.id).eq("content_id", contentId).maybeSingle();
