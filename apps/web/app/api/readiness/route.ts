...
const required = [
  ...
  "PAYMENT_WEBHOOK_SECRET", "RATE_LIMIT_SALT", "RECOMMENDATION_REFRESH_SECRET",
] as const;
...
export async function GET() {
  ...
  const names = [...required, ...(liveEnabled ? liveRequired : []), ...(drmEnabled ? drmRequired : [])];
  ...
  const ready = missing.length === 0 && database === "ready" && paymentReady && liveReady && drmReady;

  return NextResponse.json({
    service: "jalwa-web", status: ready ? "ready" : "not_ready", database,
    aiProvider: process.env.AI_PROVIDER ?? "unconfigured", paymentProvider, paymentReady, frontendPreview,
    features: { social: true, recommendations: true, live: { enabled: liveEnabled, ready: liveReady }, drm: { enabled: drmEnabled, ready: drmReady, offline: false } },
    ...
    missingConfiguration: missing, version: process.env.GIT_SHA ?? "local", time: new Date().toISOString(),
