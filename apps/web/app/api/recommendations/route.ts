...
export async function POST(request: Request) {
  ...
  const { error } = await database.rpc("record_recommendation_event", { p_viewer_profile_id: profile.id, p_content_id: body.contentId, p_event_type: body.eventType, p_session_id: body.sessionId?.slice(0,120) ?? null, p_value: Number.isFinite(body.value) ? body.value : null, p_context: body.context ?? {} });
