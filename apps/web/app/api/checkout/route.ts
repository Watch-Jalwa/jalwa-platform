...
export async function POST(request: Request) {
  ...
  const { data: orderId, error } = await database.rpc("create_checkout_order", { p_price_id: body.priceId, p_provider: provider, p_idempotency_key: idempotencyKey });
  if (error || !orderId) return NextResponse.json({ error: error?.message ?? "Checkout could not be created." }, { status: 400 });
  const { data: order } = await database.from("checkout_orders").select("amount_minor,currency").eq("id", orderId).single();
