...
export async function POST(request: Request, { params }: { params: Params }) {
  ...
  if (body.blockUserId) {
    const { error } = await supabase.rpc("set_user_block", { p_blocked_user_id: body.blockUserId, p_block: true });
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  }
  if (body.muteUserId) {
    const { error } = await supabase.rpc("set_user_mute", { p_entity_type: "user", p_entity_id: body.muteUserId, p_mute: true });
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  }
  if (body.reason) {
    ...
    const { error } = await supabase.rpc("report_comment", { p_comment_id: commentId, p_reason: body.reason, p_details: body.details?.slice(0,1000) ?? null });
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  }
