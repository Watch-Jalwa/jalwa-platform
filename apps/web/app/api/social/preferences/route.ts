...
export async function POST(request: Request) {
  ...
  if (["follow","unfollow"].includes(body.action ?? "") && body.entityType && body.entityId) {
    const { error } = await database.rpc("set_entity_follow", { p_entity_type: body.entityType, p_entity_id: body.entityId, p_follow: body.action === "follow" });
  ...
  if (["mute","unmute"].includes(body.action ?? "") && body.entityType && body.entityId) {
    const { error } = await database.rpc("set_user_mute", { p_entity_type: body.entityType, p_entity_id: body.entityId, p_mute: body.action === "mute" });
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
  ...
  if (["hide","report","share","save","like","open"].includes(body.action ?? "") && body.contentId) {
    ...
    const { error } = await database.rpc("record_recommendation_event", { p_viewer_profile_id: profile.id, p_content_id: body.contentId, p_event_type: eventType, p_session_id: null, p_value: null, p_context: body.context ?? {} });
    if (error) return NextResponse.json({ error: error.message }, { status: 400 });
