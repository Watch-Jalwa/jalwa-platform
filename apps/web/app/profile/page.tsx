...
export default async function ProfilePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (!preview && !hasBackendConfiguration()) throw new Error("Account database is not configured.");
  ...
  if (!preview) {
    ...
    const [profileResult, requestResult] = await Promise.all([
      database.from("profiles").select("display_name,preferred_language").eq("id", user.id).maybeSingle(),
      database.from("account_requests").select("id,request_type,status,requested_at,completed_at,result_expires_at,deletion_execute_after").eq("user_id", user.id).order("requested_at", { ascending: false }).limit(10),
    ]);
