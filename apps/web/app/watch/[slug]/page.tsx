  ...
  return (
    <div className="page-shell watch-layout">
      <div className="player-shell">
        {isYouTube ? <ResilientYouTubePlayer contentId={item.id} sourceUrl={item.sourceUrl} title={item.title} videoId={item.playback!.providerContentId!} /> : isSelfHosted ? <SelfHostedPlayer contentId={item.id!} poster={item.thumbnailUrl} title={item.title} /> : <div className="player-placeholder"><span className="eyebrow">{item.accessLevel === "premium" ? "Premium" : "Free"}</span><p>Playback source is awaiting media processing, rights approval or final publication.</p></div>}
      </div>
      <article className="watch-copy"><span className="eyebrow">{item.category} · {formatDuration(item.durationSeconds)}</span><h1>{item.title}</h1>{item.titleUrdu ? <p className="urdu watch-urdu">{item.titleUrdu}</p> : null}{item.description ? <p>{item.description}</p> : null}{item.attribution ? <p className="attribution">{item.attribution}</p> : null}<div className="watch-actions"><Link className="button button-secondary" href="/history">Watch history</Link>{item.sourceUrl ? <Link className="button button-secondary" href={item.sourceUrl} rel="noreferrer" target="_blank">View original source ↗</Link> : null}</div></article>
    </div>
  );
}
