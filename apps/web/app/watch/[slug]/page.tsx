...
export default async function WatchPage({ params }: { params: Params }) {
  ...
  return <div className="page-shell watch-layout">
    <div className="player-shell">{isYouTube ? <ResilientYouTubePlayer contentId={item.id} sourceUrl={item.sourceUrl} title={item.title} videoId={item.playback!.providerContentId!} /> : isDrm ? <DrmPlayer contentId={item.id!} poster={item.thumbnailUrl} title={item.title} nextPath={`/watch/${item.slug}`} /> : isSelfHosted ? <SelfHostedPlayer contentId={item.id!} poster={item.thumbnailUrl} title={item.title} /> : <div className="player-placeholder"><span className="eyebrow">{item.accessLevel === "premium" ? "Premium" : "Free"}</span><p>Playback source is awaiting media processing, rights approval or final publication.</p></div>}</div>
