  ...
  return (
    <div className="page-shell auth-journey login-journey">
      ...
      <div className="auth-methods">
        ...
        <section className="form-shell social-auth"><h2>Social sign-in</h2><p>Provider consent and account linking are handled by Supabase Auth.</p>{["google","apple","facebook"].map((provider) => <form action={startOAuth} key={provider}><input name="provider" type="hidden" value={provider} /><input name="next" type="hidden" value={next} /><button className="button button-secondary" type="submit">Continue with {provider[0].toUpperCase()+provider.slice(1)}</button></form>)}</section>
      </div>
    </div>
  );
}
