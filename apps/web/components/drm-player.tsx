...
type DrmConfig = { manifest: { hls: string | null; dash: string | null; preferred: string }; drmSystems: { widevine: { keySystem: "com.widevine.alpha"; licenseUrl: string } | null; fairplay: { keySystem: "com.apple.fps"; licenseUrl: string; certificateUrl: string } | null }; policy: { licenceDurationSeconds: number; minimumSecurityLevel: string; offlineAllowed: false } };

export function DrmPlayer({ contentId, title, poster, nextPath }: { contentId: string; title: string; poster?: string | null; nextPath: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      hls = new Hls({ enableWorker: true, emeEnabled: true, drmSystems, drmSystemOptions: { persistentState: "not-allowed", distinctiveIdentifier: "not-allowed", sessionTypes: ["temporary"], sessionType: "temporary", videoRobustness: data.policy.minimumSecurityLevel === "hardware" ? "HW_SECURE_ALL" : "SW_SECURE_DECODE" }, licenseXhrSetup(xhr) { xhr.withCredentials = true; xhr.setRequestHeader("x-jalwa-device-key", deviceKey); xhr.setRequestHeader("x-jalwa-content-id", contentId); } });
      hls.loadSource(data.manifest.hls ?? data.manifest.preferred); hls.attachMedia(video);
    ...
    void load(); return () => { cancelled = true; hls?.destroy(); };
  }, [contentId]);

  async function saveProgress(completed = false) { const video = videoRef.current; if (!video || !Number.isFinite(video.currentTime)) return; lastSavedRef.current = video.currentTime; await fetch("/api/history", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contentId, positionSeconds: completed ? video.duration : video.currentTime, durationSeconds: Number.isFinite(video.duration) ? video.duration : null, completed }), keepalive: true }).catch(() => undefined); }
  function track(video: HTMLVideoElement) { if (!Number.isFinite(video.duration) || video.duration<=0) return; const percent = video.currentTime/video.duration; for (const [threshold,eventType] of [[.25,"progress_25"],[.5,"progress_50"],[.9,"progress_90"]] as const) if (percent>=threshold && !milestonesRef.current.has(threshold)) { milestonesRef.current.add(threshold); void fetch("/api/recommendations", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ contentId, eventType }), keepalive: true }).catch(() => undefined); } }

  if (error) return <div className="player-placeholder"><p>{error.message}</p>{error.code === "payment_required" ? <Link className="button button-primary" href="/pricing">View Premium</Link> : error.code === "sign_in_required" ? <Link className="button button-primary" href={`/login?next=${encodeURIComponent(nextPath)}`}>Sign in</Link> : <button className="button button-secondary" type="button" onClick={() => window.location.reload()}>Retry</button>}</div>;
