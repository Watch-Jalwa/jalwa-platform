...
4. Set `BACKUP_AGE_IDENTITY_FILE=/opt/jalwa/secrets/backup-age.key`.
5. Set `BACKUP_AGE_RECIPIENT` to the public recipient returned by `age-keygen -y`.
6. Set `BACKUP_KEY_VERSION` to a stable identifier such as `2026-01`.
